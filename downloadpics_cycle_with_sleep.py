        ...
        for hour in range(24):
            for minute in [0, 7, 15, 22, 30, 37, 45,52 ]:
                ...
                if type == "rainfall1":           # "520153" for 2013-05-20 15:30, 
                    ...
                    if month < 10:
                        monthString = str(int(url_date[5:7]))
                    else:
                        montString = chr(month+87)  # 'a' for 10, 'b' for 11, etc
                    url = url_root + monthString + str(int(url_date[8:10])) + \
    ...
    if len(argv) > 1:
        try:
            ...
            while True:
                ...
                print 'sleeping 86400 seconds, from localtime', time.asctime()
                print 'waking up at UTC', time.asctime(time.gmtime( time.time()+ 86400 ))
                time.sleep(86400)
        except ValueError:
