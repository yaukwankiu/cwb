...
inputFolder='/media/TOSHIBA EXT/CWB/charts2/2014-05-20/'
imageName = '2014-05-20_1700.2MOS0.jpg'
outputFolder  ='/media/TOSHIBA EXT/CWB/temp/'

l   = plt.imread(inputFolder+imageName)
plt.imshow(l, origin='lower') ; plt.show()
...
plt.imshow(l6, origin='lower', cmap='jet') ; plt.colorbar() ; plt.show()
...
l4.sum()
imshow(l4, origin='lower') ; plt.colorbar() ; plt.show()
...
imshow(l8, origin='lower') ; plt.colorbar() ; plt.show()

imshow(l, origin='lower') ;plt.show()
...
plt.savefig(outputFolder+'0.jpg')    
plt.show()
...
for i in range(16):
    ...
    plt.savefig(outputFolder+str(int(time.time()))+'.jpg')
    plt.show()
